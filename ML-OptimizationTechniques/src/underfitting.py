...
# ==========================
# 1. VERİ HAZIRLIĞI (TÜM VERİ - ZOR GÖREV)
# ==========================
print("Veri setleri yükleniyor...")
df_train = pd.read_csv('training.csv')
df_test = pd.read_csv('test.csv')

# [DEĞİŞİKLİK 1]: Tüm veriyi kullanıyoruz (Zorluk seviyesi: Yüksek)
# Overfitting'de azaltmıştık, şimdi hepsini veriyoruz ki model boğulsun.
...
# ==========================
# 2. MODEL SINIFI (KAPASİTE DÜŞÜRÜLDÜ)
# ==========================
class TwoLayerMLP:
...
# ==========================
# 4. DENEY FONKSİYONU (UNDERFITTING AYARLARI)
# ==========================
def run_experiments():
    optimizers = ['gd', 'sgd', 'adam']
    seeds = [42] 
    
    # [DEĞİŞİKLİK 2]: Epoch sayısını çok AZALTIYORUZ (Vakit yok!)
    epochs = 10 
    batch_size = 32
    ...
    for opt in optimizers:
        ...
        # [DEĞİŞİKLİK 3]: Hızları düşürüyoruz (Yavaş öğrensin)
        if opt == 'gd': lr = 0.001
        elif opt == 'sgd': lr = 0.001
        else: lr = 0.0001
        ...
        for seed in seeds:
            ...
            # [DEĞİŞİKLİK 4]: Hidden Dim'i MİKROSKOBİK yapıyoruz
            # 64 veya 1024 yerine SADECE 1 NÖRON!
            # Modelin kapasitesi o kadar düşük ki hiçbir şeyi temsil edemez.
            model = TwoLayerMLP(X_train.shape[1], hidden_dim=1) 
...
# Deneyi Çalıştır
print("=" * 50)
print("UNDERFITTING DENEYİ BAŞLIYOR")
print("=" * 50)
...
for i, opt in enumerate(optimizers):
    ax = axes[i]
    # Underfitting'de HEM Train HEM Test başarısı düşük olmalı
    ax.plot(range(epochs_count), history[opt]['train_acc'], label='Train Acc (Düşük)', color='green', linewidth=2)
    ax.plot(range(epochs_count), history[opt]['test_acc'], label='Test Acc (Düşük)', color='red', linestyle='--', linewidth=2)
    
    ax.set_title(f"{opt.upper()} - Underfitting Analizi", fontweight='bold')
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Accuracy")
    # Y eksenini 0-1 arasına sabitle ki düşüklük belli olsun
    ax.set_ylim(0, 1.0) 
    ax.legend()
    ax.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('underfitting_demonstration.png', dpi=300, bbox_inches='tight')
plt.show()

print("\n✓ Underfitting grafiği kaydedildi: underfitting_demonstration.png")